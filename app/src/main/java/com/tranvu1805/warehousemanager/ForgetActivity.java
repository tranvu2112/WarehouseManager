...
public class ForgetActivity extends AppCompatActivity {
    ...
    private void forgetPassword() {
        if (Objects.requireNonNull(edtUser.getText()).toString().isEmpty() ||
            ...
            UserDTO u = MyDatabase.getInstance(this).userDAO().checkLogin(user);
            if (u.getAccount().equals(user)) {
                if (u.getEmail().equals(email)) {
                    u.setPass("1111");
                    int check = MyDatabase.getInstance(this).userDAO().updateUser(u);
                    if (check > 0) {
                        CustomDialog.dialogSingle(this, "Thông báo", "Mật khẩu đã đổi về mặc định là 1", "OK", ((dialogInterface, i) -> {
                        }));
                    } else {
